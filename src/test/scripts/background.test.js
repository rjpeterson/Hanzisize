...
describe("browserCheck", () => {
  ...
  describe("firefox", () => {
    ...
    describe("urlValid", () => {
      ...
      test("an addons url should return addons error text", () => {
        ...
        const expected = {
          message: browserCheck.firefox.addonsErrorString,
          valid: false,
        };
  ...
  describe("edge", () => {
    ...
    describe("urlValid", () => {
      ...
      test("a edge addons url should return addons error text", () => {
        ...
        const expected = {
          message: browserCheck.edge.addonsErrorString,
          valid: false,
        };
  ...
  describe("googlechrome", () => {
    ...
    describe("urlValid", () => {
      ...
      test("a webstore url should return webstore error text", () => {
        ...
        const expected = {
          message: browserCheck.googlechrome.webstoreErrorString,
          valid: false,
        };
  ...
  describe("opera", () => {
    ...
    describe("urlValid", () => {
      ...
      test("a opera addons url should return addons error text", () => {
        ...
        const expected = {
          message: browserCheck.opera.addonsErrorString,
          valid: false,
        };
  ...
  describe("getUserBrowser", () => {
    ...
    beforeEach(() => {
      spyFF = jest
        .spyOn(browserCheck.firefox, "isFirefox")
      ...
      spyGC = jest
        .spyOn(browserCheck.googlechrome, "isChrome")
        .mockReturnValue(false);
    });
  ...
  describe("checkUrl", () => {
    ...
    beforeEach(() => {
      ...
      spyFF = jest
        .spyOn(browserCheck.firefox, "urlValid")
        .mockReturnValue(resultObject);
      spyOP = jest
        .spyOn(browserCheck.opera, "urlValid")
        .mockReturnValue(resultObject);
      spyED = jest
        .spyOn(browserCheck.edge, "urlValid")
        .mockReturnValue(resultObject);
      spyGC = jest
        .spyOn(browserCheck.googlechrome, "urlValid")
        .mockReturnValue(resultObject);
    });
...
describe("scriptMethods", () => {
  ...
  describe("handleFirstMessageResponse", () => {
    ...
    test("injects scripts if no response", () => {
      response = false;
      const spy = jest
        .spyOn(scriptMethods, "handleJqueryInjection")
        .mockImplementation(() => {});
      ...
      expect(chrome.tabs.executeScript).toHaveBeenCalledWith(
        tabId,
        { file: process.env.PUBLIC_URL + "jquery-3.5.1.min.js" },
        expect.any(Function)
      );
  ...
  describe("handleJqueryInjectErr", () => {
    test("it tests for Opera Settings", () => {
      ...
      expect(scriptMethods.injectionError).toEqual(
        browserCheck.opera.popupWarning
      );
      expect(mockCallback).toHaveBeenCalledWith(
        browserCheck.opera.popupWarning,
  ...
  describe("injectContentScript", () => {
    ...
    test("script successfully injects", () => {
      const spy = jest
        .spyOn(scriptMethods, "secondMessageToScript")
        .mockImplementation(() => {});
  ...
  describe("createContentObj", () => {
    test("it combines storedData and mode into a content objetc", () => {
      const expected = {
        language: mockStoredObject.language,
        minFontSize: mockStoredObject.minFontSize,
        mode: "mode",
      };
  ...
  describe("sendToContent", () => {
    ...
    test("it send the object to the tab and passes any response to the handler function", () => {
      ...
      expect(spyCTSM).toHaveBeenCalledWith(
        tabId,
        obj,
        { frameId: 0 },
        expect.any(Function)
      );
      expect(spyHFMR).toHaveBeenCalledWith(
        chrome.runtime.lastError,
        "message response",
        tabId,
        obj,
  ...
  describe("main", () => {
    ...
    beforeEach(() => {
      ...
      spyFTI.mockImplementation(() => {
        return mockTab;
      });

      spyGUB = jest.spyOn(browserCheck, "getUserBrowser");
      spyGUB.mockImplementation(() => {
        return mockUserBrowser;
      });

      spyCU = jest.spyOn(browserCheck, "checkUrl");
      spyCU.mockImplementation(() => {
        return urlCheckResult;
      });

      spyGFS = jest
        .spyOn(chromeMethods, "getFromStorage")
        .mockReturnValue(mockStoredObject);
      spyCCO = jest
        .spyOn(scriptMethods, "createContentObj")
        .mockReturnValue(mockContentObj);
      spySTC = jest
        .spyOn(scriptMethods, "sendToContent")
        .mockReturnValue(mockCallbackReturn);
    });
    ...
    test("popup on invalid url return invalidUrlMessage object", async () => {
      ...
      const result = await scriptMethods.main(
        mode,
        typeOfCall,
        mockCallback,
        mockStoredObject
    ...
    test("popup on valid url with data supplied", async () => {
      ...
      expect(spySTC).toHaveBeenCalledWith(
        mockTab.id,
        mockContentObj,
        expect.any(Function)
      );
