// if(typeof windowtest_mode == 'undefined') {
//   hanzisizeUtil.test_mode = true
// }
...
const hanzisizeUtil = {
  ...
  getLangText(document, language, callback, newMinFontSize) {
    if(hanzisizeUtil.test_mode) {console.log(`initiating getLangText...`)}
    const langElems = [];
    ...
    for (let i = 0; i < allElems.length; i++) {
      ...
      if (el.firstChild) {
        ...
        if (hanzisizeUtil.hasLanguage(language, el.firstChild.nodeValue)) {
          ...
          if (callback) {
            if(hanzisizeUtil.test_mode) {console.log(`passing element to callback function`)}
            callback(window, language, el, newMinFontSize)
          };
        } else {
          // console log for debugging
          if(hanzisizeUtil.test_mode) {console.log(`element does not contain ${language} text`)}
        }
      }
    }
    return langElems;
  ...
  singleElementResizer(window, language, el, newMinFontSize) {
    if(hanzisizeUtil.test_mode) {console.log(`initiating singleElementResizer`)};
    ...
    const languageMatch = hanzisizeUtil.hasLanguage(language, el.firstChild.nodeValue);
      if(hanzisizeUtil.test_mode) {console.log(`language match :${languageMatch}`)};
    const currentFontSize = parseInt(window.getComputedStyle(el, null).getPropertyValue('font-size'));
      if(hanzisizeUtil.test_mode) {console.log(`current font size: ${currentFontSize}`)}
